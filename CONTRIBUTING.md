# Contributing to ep_mathjax

Thanks for helping improve `ep_mathjax` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_mathjax` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_mathjax from your local clone
pnpm run plugins i --path ../ep_mathjax

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_mathjax/` are picked up after a server restart.
...
* Etherpad version, ep_mathjax version, browser + OS
* Steps to reproduce
